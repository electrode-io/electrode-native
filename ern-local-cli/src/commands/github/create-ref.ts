...
export const commandHandler = async ({
  ...
  if (!jsApiImplsOnly) {
    const miniAppsPackages = fromBranch
      ? await cauldron.getContainerMiniAppsBranches(descriptor)
      : await cauldron.getContainerMiniApps(descriptor)
    if (miniAppsPackages.length === 0) {
      log.info(
        `No MiniApps ${
          fromBranch ? 'branches' : 'tags/sha'
        } found in Cauldron for ${descriptor}`
      )
    }
    packages.push(...miniAppsPackages)
  }
  if (!miniAppsOnly) {
    const jsApiImplsPackages = fromBranch
      ? await cauldron.getContainerJsApiImplsBranches(descriptor)
      : await cauldron.getContainerJsApiImpls(descriptor)
    log.info(
      `No JS API Implementations ${
        fromBranch ? 'branches' : 'tags/sha'
      } found in Cauldron for ${descriptor}`
    )
    packages.push(...jsApiImplsPackages)
  }
