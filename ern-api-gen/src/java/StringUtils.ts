...
function indexOfDifference(cs1, cs2?: any) {
  if (cs1 === cs2) {
...
export function camelize(word, firstToLower) {
  if (isBlank(word)) {
    return word;
  }
  ...
  const ret = word
    .replace(/(?:^\w|[A-Z]|\b\w)/g, (letter, i) =>
      firstToLower && i === 0 ? letter.toLowerCase() : letter.toUpperCase(),
    )
    .replace(/\s+/g, '');
  return ret;
}
...
export const compareTo = (value, anotherString) => {
  if (value == null && anotherString == null) {
    return 0;
  }
  if (value != null && anotherString == null) {
    return 1;
