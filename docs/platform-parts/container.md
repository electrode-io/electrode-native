...
An Electrode Native container is completely generated by Electode Native based on the MiniApps and specific native dependencies you want to include in it. The new Container must be published in a central location. Keeping it local to your machine has its benefits--for example to launch a MiniApp in the Electrode Native runner, or for development and experimentation use, but for the most part you'll want to publish your Electrode Native container so that it can be properly added to your mobile application.
