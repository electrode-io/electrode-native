...
The Electrode Native container includes the following components:

- The MiniApps   
The Miniapps are a single JavaScript bundle containing all of the JavaScript code for all MiniApps.
...
An Electrode Native container includes some code that is exposed to mobile application developers to properly integrate a container in their mobile application. This consists mostly of code to intialize the container along with some utility code to access and deal with the MiniApps stored within.

### Container publication

An Electrode Native container is completely generated by Electode Native based on the MiniApps and specific native dependencies you want to include in it. The new Container must be published in a central location. Keeping it local to your machine has its benefits--for example to launch a MiniApp in the Electrode Native runner, or for development and experimentation use, but for the most part you'll want to publish your Electrode Native container so that it can be properly added to your mobile aplication.

Electrode Native offers two publishers for the Electrode Native container: Git and Maven.

The Maven publisher is used for an Android container. Upon generation of the Electrode Native container, Maven compiles the resulting library project and publishes the resulting AAR file to a Maven repository of your choice--this can be your own custom Nexus for example.
...
A Container is versioned. Every time an Electrode Native container is re-generated, its version is updated. For the Maven publisher, the version is part of the AAR artifact. For the Git publisher, a Git tag is used for the version.

Because Electrode Native containers include some native code, they are primarily used during the development lifecycle of a mobile application version. You cannot use a newly generated Electrode Native container in a mobile application version that has already been released. When you update a Miniapp for an OTA update, a newly generated Electrode Native container is not included because you cannot ship native code OTA.

### Configuring container publication

The container publication configuration is part of the `containerGenerator` configuration object. The configuration is stored in the Electrode Native cauldron so that it can be shared across users. You should add your own configuration manually to the Electrode Native cauldron as currently there is not a command to support this.
...
#### iOS publisher remarks
The publishers array contains all the publishers you want to use to publish an Electrode Native container. In this example the Electrode Native container of the `MyWeatherApp` iOS MiniApp will be published to the GitHub repository and use Git tag for the version.
...
{container-version} is the version of the generated Electrode Native container. The container version can be in the form: `x.y.z` where x, y and z are integers. For example `1.2.3` is a vaid container version. You can  specify a version for a Container or, by default, the current version will be patched-bumped to the new version.
...
##### Manually installing a container

To manually install a container:
...
5) In Build Phases, verify that `ElectrodeContainer` is in Target Dependencies, Linke Binary With Libraries, and Embed Frameworks.
...
Initialization of a Container should ideally take place during startup of your mobile application. Ideally it should take place in your `AppDelegate.m` in `didFinishLaunchingWithOptions:` method. Otherwise, you shoud call the container initialization wherever appropriate. It's best to have it initialized as soon as possible.
...
The first parameter is an implementation of the `ElectrodePluginConfig` protocol we provide through `ElectrodeContainer` that allows you to configure for both `ElectrodeContainer` and React Native. In the sample above we use `RnDevSupportEnabled`, a static boolean constance, to decide if developer support should be enabled or not. You can adapt it for your application needs.  
...
The generated Activities are very basic, and might not fullfil more advanced needs. If you need to use your own Activity subclass to host a MiniApp, you can directly extend the `ElectrodeMiniAppActivity` class and override the methods to your needs.  

**Note** Be sure to override the following method and return the String correspoding to the MiniApp name hosted by this Activity--using the previous example, we would return "Hello".
...
The generated `UIViewController` is basic and might not fullfil your advanced needs. If you would like to use your own subclass of the `UIViewController`, you must override `viewDidLoad:` in your `UIViewController` as shown below:
