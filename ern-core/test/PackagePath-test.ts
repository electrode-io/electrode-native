...
describe('PackagePath', () => {
  const gitSshPath =
    'git+ssh://git@github.com:electrode-io/electrode-native.git'
  const gitSshVersionPath =
    'git+ssh://git@github.com:electrode-io/electrode-native.git#0.11.0'
  ...
  describe('fromString', () => {
    ...
    it('should work for a file system path without prefix', () => {
      expect(() =>
        PackagePath.fromString(fileSystemPathWithoutPrefix)
      ).to.not.throw()
    })
  ...
  describe('get isGitPath', () => {
    ...
    it('should return false for a file system package path without prefix', () => {
      expect(PackagePath.fromString(fileSystemPathWithoutPrefix).isGitPath)
        .false
    })
